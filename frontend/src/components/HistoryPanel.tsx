...
const HistoryPanel: React.FC = () => {
  ...
  return (
    <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-xl shadow-lg overflow-hidden" role="region" aria-labelledby="history-heading">
      ...
      <div className="p-3 border-b border-[var(--border-color)] bg-[var(--bg-secondary)]/50">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2" id="history-heading">
            <History className="w-4 h-4 text-[var(--color-primary-600)]" aria-hidden="true" />
            <span className="text-sm font-semibold text-[var(--text-primary)]">History</span>
          </div>
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className="p-1 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--color-primary-500)] rounded"
        ...
        <div className="flex items-center gap-2" role="group" aria-label="Undo and redo">
          <button
            onClick={undo}
            disabled={!canUndo}
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all focus:outline-none focus:ring-2 focus:ring-[var(--color-primary-500)] ${canUndo
          ...
          <button
            onClick={redo}
            disabled={!canRedo}
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all focus:outline-none focus:ring-2 focus:ring-[var(--color-primary-500)] ${canRedo
                ? 'bg-[var(--bg-primary)] text-[var(--text-primary)] hover:bg-[var(--hover-bg)] border border-[var(--border-color)]'
                : 'bg-[var(--bg-primary)]/50 text-[var(--text-secondary)]/50 cursor-not-allowed border border-transparent'
              }`}
            title="Redo (Ctrl+Y)"
        ...
        <div className="mt-2 flex items-center justify-between text-[10px] text-[var(--text-secondary)]">
          ...
          {historyItems.length > 1 && (
            <button
              ...
              className="flex items-center gap-1 text-red-500 hover:text-red-600 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 rounded px-1"
        ...
        >
          {historyItems.map((item, idx) => (
            <li
              key={idx}
              id={`history-item-${idx}`}
              className={`px-3 py-2 border-b border-[var(--border-color)]/50 last:border-0 flex items-center gap-2 text-xs cursor-pointer ${idx === currentIndex
